...
def process_chunk(sequence_data_chunk, library, aligner, q_threshold, primer1, primer2, is_reverse, chunk_id):
    ...
    for sequence_data in sequence_data_chunk:
        ...
        if start_pos2 - end_pos1 < 0 or score1 < 17 or score2 < 20:  # 引物位置不正确
            continue
...
def main(args):
    ...
    for index, ideal_seq in enumerate(library):
        primer1 = Seq(ideal_seq[:17])
        primer2 = Seq(ideal_seq[97:])
        continue

    if '_1.fq' in args.fastq_name:  # 正向
        is_reverse = False
    elif '_2.fq' in args.fastq_name:  # 反向互补
        is_reverse = True
    ...
    save_error_dict_pickle(global_error_dict, file_path=args.file_folder+args.error_file_name)


if __name__ == '__main__':
    parser = argparse.ArgumentParser()

    parser.add_argument('--library_file', type=str, default='/home/liuycomputing/wsp_sequencing/FINAL.xlsx')
    
    parser.add_argument('--file_folder', type=str, default='/home/liuycomputing/wsp_sequencing/seq_20250115/')
    parser.add_argument('--fastq_name', type=str, default='library_1.fq')
    
    parser.add_argument('--q_threshold', type=float, default=30.0)

    parser.add_argument('--error_file_name', type=str, default='whole_error_dict_read1.pkl')
