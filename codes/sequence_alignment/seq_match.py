...
def build_result_dict(library):
    result_dict = {}
    lib_len = len(library[0])
    for index, ideal_seq in enumerate(library):
        result_dict[index] = {}
        result_dict[index]['Right'] = [0] * lib_len
        result_dict[index]['Deletion'] = [0] * lib_len
        result_dict[index]['Insertion'] = [0] * lib_len
        result_dict[index]['Substitution'] = [0] * lib_len

        result_dict[index]['pileup'] = [
            {
                "A": 0, "T": 0, "C": 0, "G": 0, "N": 0
            }
            for _ in range(lib_len)
        ]
...
def match_seq_for_error(ideal_seq, tmp_seq, sub_error_dict):
...
def match_seq_for_pileup(ref_seq, query_seq, sub_pileup_list):
    ...
    aln = alignments[0]
    ref_aln, query_aln = aln.aligned

    for (r_start, r_end), (q_start, q_end) in zip(ref_aln, query_aln):
        r_pos = r_start
        q_pos = q_start

        while r_pos < r_end or q_pos < q_end:
            if r_pos < r_end and q_pos < q_end:
                base = query_seq[q_pos]
                sub_pileup_list[r_pos][base] += 1
                r_pos += 1
                q_pos += 1
            elif r_pos < r_end:
                r_pos += 1
    ...
    return sub_pileup_list

def save_result_dict_pickle(result_dict, file_path):
...
def process_chunk(sequence_data_chunk, library, aligner, q_threshold, primer_dict, is_reverse, chunk_id):
    local_result_dict = build_result_dict(library)  # 为每个线程创建独立的 result_dict
    ...
    for sequence_data in sequence_data_chunk:
        ...
        # 逐位对比，统计错误率
        local_result_dict[most_similar_index] = match_seq_for_error(
            most_similar_string, 
            sequence[start_pos1:end_pos2], 
            local_result_dict[most_similar_index]
        )
        # 逐位对比，统计pileup
        local_result_dict[most_similar_index]['pileup'] = match_seq_for_pileup(
            most_similar_string, 
            sequence[start_pos1:end_pos2], 
            local_result_dict[most_similar_index]['pileup']
        )

        # print(local_result_dict[most_similar_index])
        # check = 1
...
def main(args):
    ...
    # 初始化全局 result_dict
    global_result_dict = build_result_dict(library)
    ...
    with ProcessPoolExecutor(max_workers=num_processes) as executor:
        ...
        for future in concurrent.futures.as_completed(futures):
            local_result_dict = future.result()

            # 将各个进程的 local_error_dict 合并到 global_error_dict 中
            for index, ideal_seq in enumerate(library):
                for error_type in global_result_dict[index]:
                    if error_type != 'pileup':
                        global_result_dict[index][error_type] += np.array(local_result_dict[index][error_type])
                    else:
                        for idx, result in enumerate(local_result_dict[index]['pileup']):
                            for key in result:
                                global_result_dict[index]['pileup'][idx][key] += result[key]

    end_time = time.time()  # 记录结束时间
    print(f"Total time: {end_time - start_time} seconds")  # 输出运行时间

    save_result_dict_pickle(global_result_dict, file_path=args.output_folder+args.error_file_name)


if __name__ == '__main__':
    parser = argparse.ArgumentParser()

    parser.add_argument('--library_file', type=str, default='/home/liuycomputing/wsp_sequencing/Live-Cell-Data-Storage/test_data/ten_primers_library.xlsx')
    parser.add_argument('--primer_file', type=str, default='/home/liuycomputing/wsp_sequencing/Live-Cell-Data-Storage/test_data/ten_primers.txt')
    
    parser.add_argument('--file_folder', type=str, default='/home/liuycomputing/wsp_sequencing/Live-Cell-Data-Storage/test_data/')
    parser.add_argument('--fastq_name', type=str, default='ten_primers.fastq')
